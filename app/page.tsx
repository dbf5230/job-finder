...
export default function Home() {
  ...
  const detailFor = (job: Job) => <aside className="detail-panel inline-detail"><div className="detail-top"><span className="eyebrow">JOB DETAIL</span><button type="button" className="close-detail" onClick={() => setSelectedId(null)} aria-label="상세 닫기">×</button></div><div className="detail-company"><span className={`company-mark large ${job.isGame ? "game" : ""}`}>{job.company.slice(0, 1)}</span><div><b>{job.company}</b><span>{job.location}</span></div></div><h2>{job.title}</h2><p className="detail-summary">{job.summary}</p><div className="detail-status"><span className={isEligible(job) ? "status-ok" : "status-no"}>{isEligible(job) ? "지원 가능" : "지원 제외"}</span><span>{job.deadlineLabel}</span></div><div className="detail-grid"><div><span>개발 중심 업무</span><strong>{job.devPrimary ? "예 · 주된 업무" : "아니오"}</strong></div><div><span>모집 형태</span><strong>{job.type}</strong></div><div><span>근무 방식</span><strong>{job.mode}</strong></div><div><span>접수 상태</span><strong>{deadlineStatus(job.deadline)}</strong></div></div><section className="detail-section"><h3>주요 업무</h3><ul>{job.responsibilities.map((item) => <li key={item}>{item}</li>)}</ul></section><section className="detail-section"><h3>지원 자격</h3><ul>{job.qualification.map((item) => <li key={item}>{item}</li>)}</ul></section>{!isEligible(job) && <div className="reason-box"><b>지원 제외 이유</b><span>{exclusionReason(job)}</span></div>}<div className="detail-actions"><button type="button" className={savedAt[job.id] ? "saved-action" : ""} onClick={() => toggleSaved(job.id)}>{savedAt[job.id] ? "★ 관심 공고 저장됨" : "☆ 관심 공고 저장"}</button><a href={job.source || undefined} target="_blank" rel="noreferrer">원문 공고 보기 ↗</a></div></aside>;
  ...
  return <main className={selectedJob ? "app-shell detail-open" : "app-shell"}>
    ...
    <section className="workspace" aria-label="채용공고 탐색">
      ...
      <div className="carousel-panel"><div className="panel-heading"><div><p className="eyebrow">{view === "지원 가능" ? "APPLY NOW" : view === "제외된 공고" ? "CHECK CONDITIONS" : "SAVED JOBS"}</p><h2>{view}</h2></div><div className="sort-control"><label htmlFor="sort">정렬</label><select id="sort" value={sort} onChange={(event) => setSort(event.target.value as "priority" | "deadline")} disabled={view === "관심 공고"}><option value="priority">우선순위순</option><option value="deadline">마감일순</option></select></div></div>{activeJob ? <><div className="carousel-meta"><span>{slide + 1} / {visibleJobs.length}</span><div><button type="button" onClick={() => moveSlide(-1)} aria-label="이전 공고">←</button><button type="button" onClick={() => moveSlide(1)} aria-label="다음 공고">→</button></div></div><div className="carousel-window"><div className="carousel-track" style={{ transform: `translateX(calc(${slide} * -100%))` }}>{visibleJobs.map((job) => <div className="job-slide" key={job.id}><article className={`job-card ${job.id === activeJob.id ? "active" : ""}`} role="button" tabIndex={0} onClick={() => selectJob(job)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") selectJob(job); }}><div className="card-top"><span className={`company-mark ${job.isGame ? "game" : ""}`}>{job.company.slice(0, 1)}</span><span className={job.isGame ? "game-label" : "match-label"}>{job.isGame ? "GAME" : "MATCH"}</span></div><p className="company">{job.company}</p><h3>{job.title}</h3><div className="card-meta"><span>⌖ {job.location}</span><span>◷ {job.deadlineLabel}</span></div><div className="card-bottom"><span className="type-pill">{job.type}</span><span className="mode-pill">{job.mode}</span><button type="button" className={savedAt[job.id] ? "save-button saved" : "save-button"} onClick={(event) => { event.stopPropagation(); toggleSaved(job.id); }} aria-label={`${job.company} 관심 공고 ${savedAt[job.id] ? "해제" : "저장"}`}>{savedAt[job.id] ? "저장됨" : "관심 공고 저장"}</button></div>{view === "제외된 공고" && <p className="excluded-reason">제외: {exclusionReason(job)}</p>}</article>{selectedId === job.id && detailFor(job)}</div>)}</div></div><p className="carousel-hint">카드를 눌러 상세 내용을 확인하세요</p></> : <div className="empty"><strong>{view === "관심 공고" ? "저장한 공고가 아직 없어요" : "조건에 맞는 공고가 없어요"}</strong><span>다른 필터를 선택해보세요.</span></div>}</div>
